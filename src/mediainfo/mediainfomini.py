...
def get_media_info(input_path: str, encode_or_dl: str, uploader_name: str) -> list:
    logger.info(f"生成Mediainfo-mini中...: {input_path}")
    ...
    try:
        ...
        if file_size > 1000:
            ...
            file_size = f"{str(file_size)} GiB"
        else:
            file_size = f"{str(file_size)} MiB"
        ...
        try:
            ...
            if overallbitrate > 10000:
                ...
                if overallbitrate > 1000:
                    overallbitrate = round(overallbitrate / 1000, 2)
                    overallbitrate = f"{str(overallbitrate)} Gb/s"
                else:
                    overallbitrate = f"{str(overallbitrate)} Mb/s"
            else:
                overallbitrate = f"{str(overallbitrate)} kb/s"
        except Exception as e:
            logger.warning(e)
            overallbitrate = "未知"
            ...
            if video_track["@type"] == "Video":
                write_info_list.extend(
                    (
                        ...
                        "\n"
                        + "BIT.DEPTH...........: "
                        + encode_tracks[video_track_index]["BitDepth"]
                        + " bits",
                        "\n"
                        + "FRAME.RATE..........: "
                        + encode_tracks[video_track_index]["FrameRate"]
                        + " FPS",
                        "\n"
                        + "VIDEO...............: "
        ...
        for audio_track_index, audio_track in enumerate(encode_tracks):
            if audio_track["@type"] == "Audio":
                ...
                if "Language_String" in audio_track:
                    ...
                    logger.warning(f"音轨{str(audio_track_id)}缺少语言信息，请自行填写")
        ...
        for subtitle_track_index, subtitle_track in enumerate(encode_tracks):
            if subtitle_track["@type"] == "Text":
                ...
                if "Language_String" in subtitle_track and "Title" not in subtitle_track:
                    ...
                    logger.warning(f"字幕轨{str(subtitle_track_id)}缺少语言信息，请自行填写")
                    write_info_list.append("Ambiguous!!!")
